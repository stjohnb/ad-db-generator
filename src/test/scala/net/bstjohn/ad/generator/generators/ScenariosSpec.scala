...
class ScenariosSpec
  ...
  test("groupsJoined & acesReceived") {
    val evolution = Scenarios.recreateRealDb("test", attackerIsKerboroastable = true)
    ...
    assertEquals(domainAdminsGroup.Members.exists(_.ObjectIdentifier.contains("attacker-kerb")), true)

    val changes = diff.userChanges.find(_.userId.value.contains("attacker-kerb")).getOrElse(fail("No attacker changes"))
